 ...
 */
function wolf_enhanced_image_navigation( $url, $id ) {
...
add_filter( 'attachment_link', 'wolf_enhanced_image_navigation', 10, 2 );

/**
 * Filters wp_title to print a neat <title> tag based on what is being viewed.
 */
function wolf_wp_title( $title, $sep ) {
	global $page, $paged;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'wolf' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'wolf_wp_title', 10, 2 );
