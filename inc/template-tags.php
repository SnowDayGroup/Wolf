...
function wolf_starter_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		<h1 class="screen-reader"><?php _e( 'Post navigation', 'wolf_starter' ); ?></h1>
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'wolf_starter' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'wolf_starter' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'wolf_starter' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'wolf_starter' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'wolf_starter' ) ); ?></div>
		<?php endif; ?>
...
function wolf_starter_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'wolf_starter' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'wolf_starter' ), '<span class="edit-link">', '<span>' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'wolf_starter' ); ?></em>
					<br />
				<?php endif; ?>
